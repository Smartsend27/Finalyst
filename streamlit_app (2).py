...
def get_alpha_vantage_overview(symbol):
    ...
    if not api_key:
        logger.warning("Clé API Alpha Vantage non trouvée.")
        return None
    ...
    try:
        ...
        logger.error(f"Erreur lors de la récupération Alpha Vantage pour {symbol}: {e}")
    return None
...
def search_ticker(query):
    ...
    try:
        ...
        logger.error(f"Erreur lors de la recherche ticker pour '{query}': {e}")
        return []
...
def get_market_data(tickers):
    ...
    for name, symbol in tickers.items():
        try:
            ...
            logger.error(f"Erreur lors de la récupération des données marché pour {name} ({symbol}): {e}")
            data.append({
                "Marché": name,
                "Symbole": symbol,
...
# Other functions and UI code remain unchanged but will be refactored similarly for clarity and error handling...

# Google Analytics code injection (unchanged)
st.markdown(f"""
<head>
...
# Inject Google Analytics code invisibly
st.components.v1.html(ga_code, height=0)

# The rest of the app code continues here with improved structure, error handling, and caching as shown above.
st.markdown("""
<head>
  <link rel="manifest" href="/static/manifest.json">
  <meta name="theme-color" content="#0a9396"/>
...
def get_ai_market_advice(market_df: pd.DataFrame) -> str:
    """
    Interroge l'API Groq (modèle LLaMA) pour obtenir un conseil sur les marchés financiers
    à partir d'un DataFrame de performances boursières.
    """
    api_key = os.getenv("GROQ_API_KEY")
    if not api_key:
        return "❌ Clé API Groq non trouvée. Vérifie la variable d'environnement 'GROQ_API_KEY'."

    # Conversion du DataFrame en texte lisible par le modèle
    try:
    ...
    prompt = (
        "Tu es un expert en marchés financiers.\n"
        "Voici les performances récentes de plusieurs indices boursiers :\n\n"
        f"{table_str}\n\n"
        "En te basant sur ces données, conseille sur quel marché il serait le plus intéressant "
        "d'investir actuellement et explique pourquoi, en français, de façon concise et professionnelle."
    )
    ...
    payload = {
        "model": "meta-llama/llama-4-scout-17b-16e-instruct",  # vérifie bien que ce modèle est supporté par Groq
    ...
    try:
        ...
        response.raise_for_status()  # Déclenche une erreur en cas de réponse HTTP non 200
        ...
        return "❌ Erreur dans la structure de la réponse API (clé manquante)."
...
<style>
/* ------ FONT & BASE ------ */
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;500;700&family=Space+Grotesk:wght@500&display=swap');
...
def afficher_infos(info, titre):
    # 💡 Conseil IA
    with st.expander("💡 Conseil IA basé sur les données", expanded=False):
        with st.spinner("Analyse IA en cours..."):
            ai_key = os.getenv("GROQ_API_KEY")
            if ai_key:
                prompt = f"""Tu es un analyste financier. Voici les infos d'une entreprise :
                Nom : {info.get('shortName', 'N/A')}
                Secteur : {info.get('sector', 'N/A')}
                Prix : {info.get('currentPrice', 'N/A')} USD
                CA : {info.get('totalRevenue', 'N/A')}
                Résultat net : {info.get('netIncomeToCommon', 'N/A')}
                ROE : {info.get('returnOnEquity', 'N/A')}
                Dette : {info.get('totalDebt', 'N/A')}
                PE : {info.get('trailingPE', 'N/A')}
                Dividende : {info.get('dividendYield', 'N/A')}

                Donne une recommandation concise (Acheter, Conserver, Vendre) avec explication en français."""
                ...
                try:
                    ...
                    st.error(f"Erreur appel API IA : {e}")
            else:
                st.info("Clé GROQ_API_KEY manquante.")
...
3. Une brève explication de ton raisonnement
    ...
    # Nouvelle fonction pour visualiser le concept financier
    def visualize_financial_concept(concept):
        # Exemple simple : afficher un graphique illustrant un concept financier basique
        # Ici on crée un graphique d'exemple pour "intérêt composé" ou autre concept clé
        if "intérêt composé" in concept.lower():
            st.markdown("### Visualisation de l'intérêt composé")
            periods = list(range(1, 21))
            principal = 1000
            rate = 0.05
            values = [principal * (1 + rate) ** n for n in periods]
            df = pd.DataFrame({"Année": periods, "Montant accumulé (USD)": values})
            fig = px.line(df, x="Année", y="Montant accumulé (USD)", title="Croissance avec intérêt composé à 5% par an")
            st.plotly_chart(fig, use_container_width=True)
        elif "inflation" in concept.lower():
            st.markdown("### Visualisation de l'inflation")
            years = list(range(2000, 2021))
            inflation_rates = [2.5, 2.3, 1.6, 2.8, 3.4, 3.2, 2.5, 2.8, 3.8, 2.7, 1.5, 3.0, 2.1, 1.5, 0.8, 1.3, 2.1, 2.4, 1.9, 2.3, 1.4]
            df = pd.DataFrame({"Année": years, "Taux d'inflation (%)": inflation_rates})
            fig = px.bar(df, x="Année", y="Taux d'inflation (%)", title="Taux d'inflation annuel aux USA (2000-2020)")
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Pas de visualisation spécifique disponible pour ce concept. Affichage du texte uniquement.")
    
    # Appel de la fonction de visualisation
    visualize_financial_concept(concept_of_the_day)
